            ...
            <div class="main-container">
                <div class="pd-ltr-20">
                    <div class="title pb-20">
                        <h2 class="h3 mb-0">Leave History for faculty ID: <?php echo $emp_id; ?></h2>
                    </div>
                                    ...
                                    <?php while ($row = mysqli_fetch_assoc($query)) { ?>
                                        <tr>
                                            ...
                                            <td style="color: <?php echo ($row['Status'] == 1) ? 'green' : (($row['Status'] == 2) ? 'red' : 'blue'); ?>">
                                                <?php echo ($row['Status'] == 1) ? 'Recommended' : (($row['Status'] == 2) ? 'Not Recommend' : 'Pending'); ?></td>
                                            <td style="color: <?php echo ($row['admin_status'] == 1) ? 'green' : (($row['admin_status'] == 2) ? 'red' : 'blue'); ?>">
                                                <?php echo ($row['admin_status'] == 1) ? 'Forward' : (($row['admin_status'] == 2) ? 'Reject' : 'Pending'); ?></td>
                                            <td style="color: <?php echo ($row['principal_status'] == 1) ? 'green' : (($row['principal_status'] == 2) ? 'red' : 'blue'); ?>">
                                                <?php echo ($row['principal_status'] == 1) ? 'Approved' : (($row['principal_status'] == 2) ? 'Reject' : 'Pending'); ?></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
